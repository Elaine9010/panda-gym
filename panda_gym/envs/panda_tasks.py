...
from panda_gym.envs.tasks.pick_and_place import PickAndPlace, PickAndPlaceNew
...
class PandaPickAndPlaceNewEnv(RobotTaskEnv):    #new task
    """Pick and Place task wih Panda robot.
    ...
    def __init__(
        ...
        render_pitch: float = -30,
        render_roll: float = 0,
        interest_object: str = "object_0",
        target_object: str = "object_1",
    ) -> None:
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = PickAndPlaceNew(sim, reward_type=reward_type, interest_object=interest_object, target_object=target_object)
        super().__init__(
            robot,
            task,
            render_width=render_width,
